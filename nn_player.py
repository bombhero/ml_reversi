...
class ReversiCriticNet(torch.nn.Module):
    def __init__(self):
        super(ReversiCriticNet, self).__init__()
        self.conv_l1 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=4, out_channels=256, kernel_size=8, padding=0),
...
setattr(__main__, 'ReversiCriticNet', ReversiCriticNet)


class AIPlayer:
    def __init__(self, player_name, color, oppo_color, model_file_path=None):
...
if __name__ == '__main__':
    tst_player = AIPlayer('AIPlayer', 1, -1)
